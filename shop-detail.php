<?php
    ...
    if (isset($_GET) && $_GET && $_GET['id']){
        $sql = "SELECT product.id, product.title, description, prix, image_src, categorie.title as categorie_title FROM `product` INNER JOIN categorie on id_categorie = categorie.id WHERE product.id = " . $_GET['id'];
    ...
    if (isset($_POST) && $_POST) {
        $sql = "INSERT INTO `review` (`id_user`, `id_product`, `rating`, `comment`) VALUES ('1', '" . $_GET['id'] . "', '" . $_POST['rating'] . "', '" . $_POST['review'] . "');";
    ...
    $sql = "SELECT rating, comment, date, name, image_src FROM `review` INNER JOIN user on id_user = user.id where id_product = " . $_GET['id'] . " ORDER by date desc;";
    $result = $conn->query($sql);
    $conn->close();
?>
<!DOCTYPE html>
                ...
                <div class="col-lg-8 col-xl-9">
                    <div class="row g-4">
                        ...
                        <div class="col-lg-6">
                            ...
                            <p class="mb-3">Category: <?= $product['categorie_title'] ?></p>
                            <h5 class="fw-bold mb-3"><?= $product['prix'] ?> DT / KG</h5>
                                    ...
                                    <?php 
                                        if ($result->num_rows > 0) {
                                            ...
                                            while($row = $result->fetch_assoc()) { ?>
                                                <div class="d-flex">
                                                    <img src="<?= $row['image_src'] ?>" class="img-fluid rounded-circle p-3" style="width: 100px; height: 100px;" alt="">
                                                    <div class="">
